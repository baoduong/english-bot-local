...
def _seed_practice_session(*, user_id: str = "fault-wav2vec2-user", sentence: str = "fix") -> str:
...
async def test_empty_alignment_degrades_gracefully(client, clean_db, mock_whisper, monkeypatch: pytest.MonkeyPatch):
    del clean_db, mock_whisper
    ...
    response = await client.post(
        "/practice/audio",
        data={"user_id": "fault-wav2vec2-user"},
    ...
    assert body["scoring"]["word_scores"][0]["phoneme_match_ratio"] is None


@pytest.mark.asyncio
@pytest.mark.fault_injection
async def test_wav2vec2_exception_doesnt_crash(client, clean_db, mock_whisper, monkeypatch: pytest.MonkeyPatch):
    del clean_db, mock_whisper
    _seed_practice_session()
    _stub_dependencies(monkeypatch)
    from api.routers import practice

    def _raise_alignment_failure(*_args):
        raise RuntimeError("Model not loaded")

    monkeypatch.setattr(practice, "analyze_phonemes_per_word", _raise_alignment_failure)
    ...
    assert body["scoring"]["engine"] == "whisper"
    assert body["scoring"]["word_scores"]
